...
function render()
{
    ...
    rotDay += 1.0 + Dayup;
    rotYear += 0.1 + Yearup;        // not the correct value, but looks better!
	...
	//teikna "gervitunglið"
	mvt = mult( mv, rotateY( 30.0*rotYear ) );
	mvt = mult( mv, rotateZ( rotYear*10 ) );
	mvt = mult( mvt, translate( 1.2, 0.0, 0.0 ) );
	mvt = mult( mvt, scalem( 0.2, 0.2, 0.2 ) );
	gl.uniformMatrix4fv(matrixLoc, false, flatten(mvt));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
